...
const AdminRuleBookTypes: FC = () => {
  ...
  const onClickUpdate = useCallback((_id: string, name: string) => {
    setValue('name', name);
    setUpdateBookTypeMode(_id);
  }, [setValue]);

  const onClickDeletion = useCallback((_id: string) => {
    if (api !== undefined) {
      if (deleteBookTypeMode === '') {
        ...
        api.ruleBookTypes.delete({ id: _id })
          .then(() => {
            const newId = getNewId();
            createAlert({
              key: newId,
  ...
  const rulebookTypesDom = useMemo(() => ruleBookTypes.map(({ name, _id }) => (
    <Ali key={_id} className="adminRuleBooktypes__list__elt">
      <Ap>{t(`ruleBookTypeNames.${name}`, { count: 1 })}{` (${name})`}</Ap>
      <div className="adminRuleBooktypes__list__elt__buttons">
        <Button
          size="small"
          onClick={() => { onClickUpdate(_id, name); }}
          icon="edit"
        />
        <Button
          size="small"
          onClick={() => { onClickDeletion(_id); }}
          icon="delete"
        />
      </div>
    </Ali>
  ...
  const createUpdateRuleBookTypesText = useMemo(() => {
    ...
    return createBookTypeMode ? t('adminRuleBooks.saveType', { ns: 'pages' }) : t('adminRuleBooks.createType', { ns: 'pages' });
  ...
  return (
    <div className="adminRuleBooktypes">
      <Atitle level={2}>{i18next.format(t('terms.ruleBook.type', { count: ruleBookTypes.length }), 'capitalize')}</Atitle>
      {
        deleteBookTypeMode !== ''
          ? (
            <Ap className="adminRuleBooktypes__alert">{t('adminRuleBooks.deleteRuleBookTypeMessage', { ns: 'pages' })}</Ap>
            )
          : null
      }
      ...
      <div className="adminRuleBooktypes__create">
        ...
        <div className="adminRuleBooktypes__create__buttons">
          <Button
            size="small"
            onClick={createBookTypeMode || updateBookTypeMode !== '' ? handleSubmit(onSubmit) : () => { setCreateBookTypeMode(true); }}
          >
