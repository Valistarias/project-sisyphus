...
import type { ICuratedCharParam, ICuratedNode, ICuratedSkill, ICuratedStat } from '../types';
          ...
          <div className="node__content__bonuses">
            <Aul className="node__content__bonuses__list">
              {statBonuses.map(({ value, stat }) => (
                <Ali className="node__content__bonuses__elt" key={`stat-${stat?.stat._id}`}>
                  {`+${value} ${stat?.stat.title}`}
                </Ali>
              ))}
              {skillBonuses.map(({ value, skill }) => (
                <Ali className="node__content__bonuses__elt" key={`skill-${skill?.skill._id}`}>
              ...
              {charParamBonuses.map(({ value, charParam }) => (
                ...
                >
                  {`+${value} ${charParam?.charParam.title}`}
                </Ali>
              ))}
            </Aul>
          </div>
