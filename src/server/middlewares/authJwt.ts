...
const verifyToken = (
  ...
  jwt.verify(session.token, secret, (err, decoded?: jwt.JwtPayload & { id: string }) => {
...
const generateVerificationMailToken = (userId: string): string | null => {
  ...
  const verificationToken = jwt.sign({ IdMail: userId }, secret, { expiresIn: '7d' });

  return verificationToken;
};

const adminNeeded = (req: IVerifyTokenRequest, res: Response, next: () => void): void => {
  isAdmin(req)
    .then((boolCheck) => {
      if (boolCheck) {
        next();
