...
const findWeapons = async (options?: findAllPayload): Promise<LeanIWeapon[]> =>
...
const update = (req: Request, res: Response): void => {
  ...
  findCompleteWeaponById(id)
    .then((weapon) => {
      ...
      if (effects !== null) {
        effectsToRemove = weapon.effects.reduce((result: string[], elt: HydratedIEffect) => {
      ...
      if (actions !== null) {
        actionsToRemove = weapon.actions.reduce((result: string[], elt: HydratedIAction) => {
      ...
      if (i18n !== null) {
        const newIntl: InternationalizationType = {
          ...(weapon.i18n !== undefined && weapon.i18n !== '' ? JSON.parse(weapon.i18n) : {}),
        };
...
const deleteWeapon = (req: Request, res: Response): void => {
  ...
  findCompleteWeaponById(id)
    .then(
      (
        weapon: Omit<HydratedIWeapon, 'effects' | 'actions' | 'damages'> & {
...
export interface CuratedIWeaponToSend {
  weapon: Omit<LeanIWeapon, 'effects' | 'actions' | 'damages'> & {
    damages: IDamage[];
    effects: Array<{
      effect: IEffect;
...
export const curateSingleWeapon = (weaponSent: LeanIWeapon): CuratedIWeaponToSend => {
